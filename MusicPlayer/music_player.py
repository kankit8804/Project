...
#Libraries 
import os                       # to select the music file and display there name 
import pickle                   #
import tkinter as tk            # to create GUI
from tkinter import filedialog  # to open the file dialog to load the music 
from tkinter import PhotoImage
from webbrowser import BackgroundBrowser  # to display the image that we arer using 
from pygame import mixer        # to control the music system paly, pause, forward 
...
class Player(tk.Frame):
	def __init__(self, master=None):
       ...
       #Create frames for maping 
	def create_frames(self):
		self.track = tk.LabelFrame(self, text='Song Track', 
					font=("Arial",15,"bold"),
					bg="black",fg="white",bd=5,relief=tk.RIDGE)
		...
		self.tracklist = tk.LabelFrame(self, text=f'PlayList - {str(len(self.playlist))}',
							font=("Arial",15,"bold"),
							bg="black",fg="white",bd=5,relief=tk.RIDGE)
		self.tracklist.config(width=240,height=500)
		self.tracklist.grid(row=0, column=1, rowspan=3, pady=5)

		self.controls = tk.LabelFrame(self,
							font=("Arial",15,"bold"),
							bg="white",fg="black",bd=2,relief=tk.GROOVE)
	...
	def track_widgets(self):
		...
		self.songtrack = tk.Label(self.track, font=("Arial",16),
						bg="black",fg="white")
		self.songtrack['text'] = 'Serenity MP3 Player'
...
root = tk.Tk()                 # it will create a small window 
root.geometry('600x400')
root.wm_title('Serenity MP3 Player')

img = PhotoImage(file='images/home_wallpaper.gif')  #this will load image 
